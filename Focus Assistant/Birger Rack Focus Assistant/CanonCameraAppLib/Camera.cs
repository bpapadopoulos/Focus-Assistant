...
namespace CanonCameraAppLib
{
    ...
    public sealed class Camera
    {
                ...
                case EDSDK.PropertyEvent_PropertyChanged:
                    switch (inPropertyID)
                    {
                        case EDSDK.PropID_Evf_OutputDevice:
                            invokePropertyChangedEvent(new PropertyChangedEventArgs());
                            break;
        ...
        public double CurrentAperture
        {
            get
            {
                if (ApertureValues.Count > 0)
                {
                    uint data = 0;
                    uint error;
                    error = EDSDK.EdsGetPropertyData(this.CameraDevice, EDSDK.PropID_Av, 0, out data);

                    if (EDSDK.EDS_ERR_OK != error)
                    {
                        throw new PropertyUnavailableException("Unable to get aperture value.", error);
                    }

                    if (data != 0)
                        return cameraApi.ApertureDictionary[(int)data];
                    else
                        return 0;
                }
                else
        ...
        public List<double> ApertureValues
        {
            get
            {
                return refreshApertureValues();
            }
        }
        ...
        /// <summary>
        /// Does the camera currently have a session opened?
        /// </summary>
        ...
        public bool IsInLiveViewMode
        {
            get
            {
                ...
                try
                {
                    error = EDSDK.EdsGetPropertyData(this.CameraDevice, EDSDK.PropID_Evf_OutputDevice, 0, out data);

                    if (EDSDK.EDS_ERR_OK != error)
                    {
                        throw new PropertyUnavailableException("Unable to get live view status.", error);
                    }
                }
                catch (Exception ex)
                {
                ...
                return data != (uint)LiveViewDevice.None;
            }
        }
        #endregion
        ...
        public uint DriveLensNearOne()
        {
            return EDSDK.EdsSendCommand(this.CameraDevice, EDSDK.CameraCommand_DriveLensEvf, (int)EDSDK.EvfDriveLens_Near1);
        }

        public uint DriveLensNearTwo()
        {
            return EDSDK.EdsSendCommand(this.CameraDevice, EDSDK.CameraCommand_DriveLensEvf, (int)EDSDK.EvfDriveLens_Near2);
        }

        public uint DriveLensNearThree()
        {
            return EDSDK.EdsSendCommand(this.CameraDevice, EDSDK.CameraCommand_DriveLensEvf, (int)EDSDK.EvfDriveLens_Near3);
        }

        public uint DriveLensFarOne()
        {
            return EDSDK.EdsSendCommand(this.CameraDevice, EDSDK.CameraCommand_DriveLensEvf, (int)EDSDK.EvfDriveLens_Far1);
        }

        public uint DriveLensFarTwo()
        {
            return EDSDK.EdsSendCommand(this.CameraDevice, EDSDK.CameraCommand_DriveLensEvf, (int)EDSDK.EvfDriveLens_Far2);
        }

        public uint DriveLensFarThree()
        {
            return EDSDK.EdsSendCommand(this.CameraDevice, EDSDK.CameraCommand_DriveLensEvf, (int)EDSDK.EvfDriveLens_Far3);
        }

        public bool SetAperture(int key)
        {
            uint error;

            if (this.ApertureValues.Count > 1)
            {
                error = EDSDK.EdsSetPropertyData(this.CameraDevice, EDSDK.PropID_Av, 0, Marshal.SizeOf((uint)cameraApi.ApertureInverseDictionary[key]), (uint)cameraApi.ApertureInverseDictionary[key]);
        ...
        private void getLensName()
        {
            string data = string.Empty;
            uint error = 0;

            error = EDSDK.EdsGetPropertyData(this.CameraDevice, EDSDK.PropID_LensName, 0, out data);
            
            if (EDSDK.EDS_ERR_OK == error)
            {
                lensName = data.Replace("/", "").Replace("\\", "");
            }
        }

        private uint getEVFMode()
        {
            uint data = 0;
            uint error;
            error = EDSDK.EdsGetPropertyData(this.CameraDevice, EDSDK.PropID_Evf_Mode, 0, out data);
        ...
        private List<double> refreshApertureValues()
        {
            List<double> apertureValues = new List<double>();
            EDSDK.EdsPropertyDesc apertureDesc = new EDSDK.EdsPropertyDesc();
            uint error = 0;
            Dictionary<int, int> inverseDictionary = new Dictionary<int, int>();

            Stopwatch stopWatch = new Stopwatch();
            stopWatch.Start();
            while (apertureDesc.NumElements < 1 && stopWatch.ElapsedMilliseconds < 50)
            {
                error = EDSDK.EdsGetPropertyDesc(this.CameraDevice, EDSDK.PropID_Av, out apertureDesc);
            }
            if (EDSDK.EDS_ERR_OK == error)
            {
                for (int x = 0; x < apertureDesc.NumElements; x++)
                {
                    apertureValues.Add(cameraApi.ApertureDictionary[apertureDesc.PropDesc[x]]);
                    inverseDictionary.Add(x, apertureDesc.PropDesc[x]);
                }

                cameraApi.ApertureInverseDictionary = inverseDictionary;
            }
